...
# Dropping columns
columns_to_drop <- c(
    "ID", "Name", "Source_of_Recruitment", "Retired",
    "Last_Education_Subject", "Last_Education_Division",
    "Last_Designation", "Last_Level", "Last_Office",
    "Last_Field_of_Experience", "Last_Category_of_Experience",
    "Source", "Gender_Source"
)
profile_data <- subset(profile_data, select = -columns_to_drop)

# Find median for 'Allotment_Year' and replace NAs
...
profile_data$Languages_Known <- ifelse(
    is.na(profile_data$Languages_Known) |
        profile_data$Languages_Known == "",
    ifelse(!is.na(profile_data$Mother_Tongue) &
...
mode_languages_known <- as.character(
    names(sort(table(profile_data$Languages_Known[profile_data$Languages_Known != ""]), decreasing = TRUE)[1])
)
...
# Create a bar plot
barplot(mother_tongue_counts_df$Count,
...
text(
    x = barplot(mother_tongue_counts_df$Count,
        names.arg = mother_tongue_counts_df$Mother_Tongue,
        col = "red"
    ) + 0.25, # Adjust 0.25 to set the position of labels
...
ggplot(language_count_summary, aes(x = factor(Languages_Count), y = Count)) +
    ...
    labs(
        title = "Number of Candidates Knowing Different Numbers of Languages",
